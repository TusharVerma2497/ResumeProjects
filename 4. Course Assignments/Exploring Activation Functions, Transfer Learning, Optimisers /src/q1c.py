...
#(c) (Role of optimizer) Fix the number of epochs, learning rate, batch size, activation
# function, train this network first using the following optimizers:
...
list_of_optims=[optimizer=torch.optim.Adam, torch.optim.SGD, torch.optim.SGD, torch.optim.Rprop]
# list_of_optims=[torch.optim.Rprop]
list=[]
flag = False
for i in list_of_optims:
    model=VGG16(activationFunction=nn.ReLU)
    model=model.to(device)
    #optimizer
    if(i==torch.optim.SGD and flag==True):
        print('inside')
        optimizer=i(model.parameters(), lr=0.01, momentum=0.1)
        flag=False
    else:
        print('outside')
        optimizer=i(model.parameters(), lr=0.01,)
        flag=True
...
labels=["Adam (lr=0.01)", "Vanilla Gradient descent (lr=0.01)", "Momentum based gradient descent (lr=0.01, momentum=0.1)", "RMSProp (lr=0.01)"]
