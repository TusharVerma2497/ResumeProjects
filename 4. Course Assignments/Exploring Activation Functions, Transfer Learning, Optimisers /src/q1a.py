...
#(a)
# (Which Activation function is better?) Fix the number of epochs (to 50 epochs) and
# fixate any learning rate, optimizer, batch size, train this network, with layers having:
...
list_of_activationFunctions=[nn.ReLU, nn.LeakyReLU, nn.Tanh, nn.Sigmoid]
list=[]
for i in list_of_activationFunctions:
    model=VGG16(activationFunction=i)
...
labels=["ReLU", "LeakyReLU", "Tanh", "Sigmoid"]
#training accuracy plot of different activation functions
for i in range(len(list_of_activationFunctions)):
